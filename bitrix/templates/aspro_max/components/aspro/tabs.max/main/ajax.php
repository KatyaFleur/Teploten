...
$iblock_id = 1; 
$arFilter = Array("IBLOCK_ID"=>$iblock_id, "ACTIVE"=>"Y", array("ID" => CIBlockElement::SubQuery("ID", array("IBLOCK_ID" => 1, "PROPERTY_HIT" => 713))),);
$res_count_best_price = CIBlockElement::GetList(Array(), $arFilter, Array(), false, Array());



$arFilter = Array("IBLOCK_ID"=>$iblock_id, "ACTIVE"=>"Y", array("ID" => CIBlockElement::SubQuery("ID", array("IBLOCK_ID" => 1, "PROPERTY_HIT" => 708))),);
$res_count_lider = CIBlockElement::GetList(Array(), $arFilter, Array(), false, Array());
					...
					<div class="right_block_wrapper">
						<div class="tabs_wrapper <?=$arParams['TITLE_BLOCK_ALL'] && $arParams['ALL_URL'] ? 'with_link' : ''?>">
							<ul class="tabs ajax">
								...
								<li data-code="<?=$code?>" class="font_xs <?=($i==1 ? "cur clicked" : "")?>"><span class="muted777"><?=$arTab["TITLE"];?>
								<?php if($i == 1): ?>
									<a class="hit-count" href="/vygodnaya_tscena/"><?echo ' (' . $res_count_best_price . ')';?></a>
								<?php elseif($i == 2): ?>
									<a class="hit-count" href="/lider_prodazh/"><?echo ' (' . $res_count_lider . ')';?></a><?endif;?></span></li>
									<?$i++;?>
								<?endforeach;?>
							</ul>
						</div>
