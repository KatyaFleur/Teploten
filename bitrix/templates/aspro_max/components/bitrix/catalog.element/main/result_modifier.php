...
if($arParams['LINKED_FILTER_BY_PROP']['EXPANDABLES'] || $arParams['LINKED_FILTER_BY_FILTER']['EXPANDABLES'])
{
	...
	/*Добавочка*/
} elseif(!empty($arResult['PROPERTIES']['RECOMEND']['VALUE'])) {
	$arResult['EXPANDABLES']['IBLOCK_ID'] = $arParams['IBLOCK_ID'];
	$arResult['EXPANDABLES'] = array('ID' => $arResult['PROPERTIES']['RECOMEND']['VALUE']);
} else {
	$similar = array();
	$arResult['ASSOCIATED']['IBLOCK_ID'] = $arParams['IBLOCK_ID'];
	$arFilter = array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'SECTION_ID' => $arResult["IBLOCK_SECTION_ID"],"INCLUDE_SUBSECTIONS" => "Y",'CATALOG_AVAILABLE' =>'Y', 'ACTIVE' => 'Y', '!ID' => $elem['ID']);
	$res = CIblockElement::GetList(Array("RAND" => "ASC"), $arFilter, false, Array("nPageSize"=>10), array('IBLOCK_ID','ID'));
	while($arFields = $res->GetNext()){
		$similar[] = $arFields['ID'];
	}
	$arResult['ASSOCIATED'] = array('ID' => $similar);
}
