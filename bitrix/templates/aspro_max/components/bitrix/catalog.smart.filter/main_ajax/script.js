...
JCSmartFilter.prototype.deleteLink = function(del_link)
{
  if(!!this.timer)
  {
    clearTimeout(this.timer);
  ...
  var id = $(del_link).data('id'),
    link = BX(id),
    set_filter = BX('set_filter');
  if(!link) link = BX(id+'Y');
  link.checked = false;
  $(del_link).fadeOut('200');
  BX.addClass( set_filter, 'reloaded' );
    ...
    success: function (html) {
      ...
      if (reset && (!$("#wrapInlineFilter").length || !$("#wrapInlineFilter").is(":empty"))) {
        ...
        var itemsHtml = $.trim($(html).find(".inner_wrapper").html()),
          clear_filters = $(".tags_filter_inner");
        $(".middle > .container .inner_wrapper").html(itemsHtml);
        if(clear_filters.length){
          clear_filters.html($.trim($(html).find(".tags_filter_inner").html()));
        }
        //ajax top-block
        $(".top-content-block").html($(html).find(".top-content-block").html());
        ...
        if (reset && $("#mobilefilter .bx_filter").length) {
          ...
          if($(window).width() < 1200){
            ...
            $("#mobilefilter .bx_filter_parameters").addClass("scrollblock");
          }
          ...
          console.log($("#mobilefilter .bx_filter_parameters"));
          InitCustomScrollBar();
